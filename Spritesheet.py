...
class Spritesheet:
    def __init__(self, filepath: Path, sprite_size: Tuple[int, int], rotation = False, spacing: Tuple[int, int] = (0, 0),
    ...
    def get_sprite(self, loc: Tuple[int, int], colorkey: Union[pygame.Color, int, None] = None) -> pygame.Surface:
        ...
        """
        x = loc[0] * (self._sprite_size[0] + self._spacing[0])
        y = loc[1] * (self._sprite_size[1] + self._spacing[1])
    ...
    def get_sprites(self, locs: List[Tuple[int, int]], colorkey: Union[pygame.Color, int, None] = None) -> List[
        pygame.Surface]:
